...
class SocialController extends Controller
{
    ...
    public function getSocialRedirect($provider)
    {
        ...
        if (empty($providerKey)) {
            return view('partials.status')->with('error', 'No such provider');
    ...
    public function getSocialHandle($provider)
    {
        ...
        $userCheck = Multi::where('email', '=', $user->email)->first();
        $email = $user->email;
        ...
        if (!empty($userCheck)) {
            ...
            $sameSocialId = SocialLogins::where('social_id', '=', $user->id)
        ...
        if (auth()->user()->hasRole('admin') || auth()->user()->hasRole('manager') || auth()->user()->hasRole('vmi_user')) {
